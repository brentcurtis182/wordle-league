...
def get_scores_for_wordle_by_league(wordle_number, league_id):
    ...
    try:
        ...
        for row in result:
            ...
            logging.info(f"Found score for {name}: {score_display}, emoji pattern: {'Yes' if processed_emoji_pattern else 'No'}")
...
def generate_week_pages():
    """Generate week pages with actual data"""
    ...
    week_data = {
        'aug-4th-(14)': {
            ...
            'winners': []  # Will be populated later if needed
        }
    }
    
    # Generate week page
    output_path = os.path.join(WEEKS_DIR, "aug-4th-(14).html")
    with open(output_path, 'w', encoding='utf-8') as f:
        html_content = template.render(**week_data['aug-4th-(14)'])
        f.write(html_content)
    logging.info(f"Generated week page for August 4th")
    
    return week_data

def generate_day_pages_with_real_data(league_id=1):
    """Generate day pages for Wordle games 1507-1513 with real data from database"""
    logging.info(f"Generating day pages with real data for league {league_id}")
    ...
    for wordle_num in range(1507, 1514):
        ...
        logging.info(f"Found {len(scores)} scores for Wordle #{formatted_num}")
        
        # Prepare data for the day page
        day_data = {
            'title': f'Wordle #{formatted_num} - {wordle_date}',
        ...
        # Generate the page
        output_path = os.path.join(DAYS_DIR, f"wordle-{wordle_num}.html")
        with open(output_path, 'w', encoding='utf-8') as f:
            html_content = template.render(**day_data)
            f.write(html_content)
        logging.info(f"Generated day page for Wordle #{formatted_num} with {len(scores)} real scores")
...
def main():
    ...
    for league in config.get('leagues', []):
        ...
        generate_day_pages_with_real_data(league_id)
