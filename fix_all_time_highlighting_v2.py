...
Ensures that highlighting is consistent across the entire row with the correct brightness
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("fix_highlighting_v2.log"),
            ...
            if all_time_table:
                ...
                for row in all_rows:
                    ...
                    if 'style' in row.attrs and 'background-color' in row['style']:
                        # Remove any existing style from the row
                        old_style = row['style']
                        row['style'] = HIGHLIGHT_STYLE
                        
                        # Remove any existing background-color style from all cells
                        for cell in row.find_all('td'):
                            if 'style' in cell.attrs:
                                cell_style = cell['style']
                                if 'background-color' in cell_style:
                                    # Remove just the background-color property
                                    cell_style = re.sub(r'background-color:[^;]+;', '', cell_style)
                                    if cell_style.strip():
