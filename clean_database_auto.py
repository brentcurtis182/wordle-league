...
def clean_scores_table():
    ...
    try:
        ...
        print(f"Before cleaning: {before_count} scores in database")
        ...
        print(f"After cleaning: {after_count} scores in database")
        
        if after_count == 0:
            logging.info("Successfully cleaned scores table")
            print("Successfully cleaned scores table")
            return True
        else:
            logging.error("Failed to clean scores table completely")
            print("Failed to clean scores table completely")
            return False
            
    except Exception as e:
        logging.error(f"Error cleaning scores table: {e}")
        print(f"Error cleaning scores table: {e}")
        if conn:
            conn.rollback()
        return False
    finally:
...
def main():
    ...
    print("Creating database backup first...")
