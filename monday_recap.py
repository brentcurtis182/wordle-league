    ...
    try:
        ...
        if is_division_mode:
            ...
            for div_num in (1, 2):
                ...
                # Check if a division season was just clinched (supports co-champions)
                cursor.execute("""
                ...
                if recent_winners:
                    ...
                    clinch_info = {
                        ...
                        'name': ' and '.join(names),
                        'names': names,
                        'wins': recent_winners[0][1],
                        'season_number': recent_winners[0][2],
                        'co_champions': len(names) > 1,
                    }
                    # Div II winner(s) get promoted
                    if div_num == 2:
...
5. Never state a win count, ordinal, or total that is not written in the scenario.
   Do not add up, extrapolate, or guess how many wins anyone has."""
        ...
        # Add league URL
        league_url = f"https://{os.environ.get('APP_DOMAIN', 'app.wordplayleague.com')}/leagues/{results['league_slug']}"
